-- Supabase Database Setup for Workspace Functionality
...
VALUES (
    ...
    (SELECT id::UUID FROM organizations WHERE name ILIKE '%twistworld%' OR name ILIKE '%twist%' LIMIT 1)
...
VALUES (
    (SELECT id::UUID FROM users WHERE email = 'admin@twistworld.co.uk' LIMIT 1),
    'twistworld-org',
    'owner',
    true
...
-- Function to check workspace member limits
...
BEGIN
    ...
    SELECT 
        COUNT(*),
        COALESCE(ws.seat_limit, 5)
    INTO 
        current_count,
        member_limit
    FROM user_workspaces uw
    LEFT JOIN workspace_settings ws ON ws.workspace_id = uw.workspace_id
    ...
    IF current_count >= member_limit THEN
        RAISE EXCEPTION 'Workspace member limit (%) exceeded. Current members: %', member_limit, current_count;
    END IF;
...
-- Create trigger for member limit checking
CREATE TRIGGER check_member_limit_before_insert
    BEFORE INSERT ON user_workspaces
...
-- Verify admin user has workspace access
SELECT 
    u.email,
...
-- 1. Your workspace tables will be created with proper constraints
...
-- 4. Automatic triggers will handle timestamps and member limits
-- 5. Your workspace functionality should work without 500 errors